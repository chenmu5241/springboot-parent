	...
	<div class="col-xs-12" id="content">
		...
		<form class="form-horizontal" role="form"  id="form" method="post" action=""   >
			...
			<div class="form-group col-xs-6">
				...
				<div class="col-sm-10">
					<input type="text"  placeholder="配置名称" name="keyName" value="" data-rule="required;remote[${ctx}/settingsConfig/checkSettingsConfig]"  class="col-xs-8" />
				</div>
			</div>
			<div class="form-group col-xs-6">
				<label class="col-sm-2 control-label no-padding-right" for="form-field-1">配置值<span class="red">*</span></label>
				<div class="col-sm-10">
					<input type="text" name="keyValue"  placeholder="配置值" data-rule="required"  class="col-xs-8" />
				</div>
			</div>
			<div class="form-group col-xs-6">
				<label class="col-sm-2 control-label no-padding-right" for="form-field-1">是否只读</label>
				<div class="col-sm-10">
					<select name="rwValue"  class="col-xs-8"  style="padding:0">
			...
			<div class="form-group col-xs-6">
				...
				<div class="col-sm-10">
					<input type="text" name="description"  placeholder="配置描述"  class="col-xs-8" />
				</div>
			</div>
			<div class="col-xs-12">
...
<!-- /.row -->
<script>
appendBreadcrumb("#settingsConfig/toList","新增");

function submit(){
	if(!$("form").isValid()){
		return;
	}
	bootbox.confirm("增加后配置项名称无法修改,确定要增加吗?",function(f){
		if(f){
			$.ajax({
				type : "post",
				cache : false,
