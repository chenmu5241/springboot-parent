	...
	<div class="col-xs-12" id="contentc">
		...
		<form class="form-horizontal" role="form"  id="form" method="post" action="">
			...
			<div class="form-group col-xs-6">
				...
				<div class="col-sm-10">
					<input type="text"  placeholder="权限名称" name="name" value="${power.name }"  readonly="readonly"   class="col-xs-8" />
				</div>
			</div>
			<div class="form-group col-xs-6">
				<label class="col-sm-2 control-label no-padding-right" for="form-field-1">权限标识</label>
				<div class="col-sm-10">
					<input type="text" name="flag"  value="${power.flag }"" placeholder="权限标识"   readonly="readonly"   class="col-xs-8" />
				</div>
			</div>
			<div class="form-group col-xs-6">
				...
				<div class="col-sm-10">
					<select name="parentId"  class="col-xs-8"  style="padding:0">
						<option value="-1">请选择</option>
							<c:forEach items="${powers }" var="temp">
								<option value="${temp.id }"  layer="${temp.layer }" <c:if test="${power.parentId == temp.id }">selected="true"</c:if>>${temp.name }</option>
							</c:forEach>
					</select>
				</div>
			</div>
			<div class="form-group col-xs-6">
				...
				<div class="col-sm-10">
					<select name="isShow"  class="col-xs-8"  style="padding:0">
							<option value="0" <c:if test="${power.isShow == 0}">selected="true"</c:if>>否</option>
							<option value="1"  <c:if test="${power.isShow == 1}">selected="true"</c:if>>是</option>
					</select>
				</div>
			</div>
			<div class="form-group col-xs-6">
...
<!-- /.row -->
<script>
appendBreadcrumb("#power/toList","编辑");
